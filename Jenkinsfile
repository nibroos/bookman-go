pipeline {
  agent any
  environment {
    GIT_REPO = 'git@github.com:nibroos/bookman-go.git'
    SSH_CREDENTIALS_ID = 'vps-ssh-credentials-bookman-27'
    VPS_USER = credentials('vps-user-27')
    VPS_HOST = credentials('vps-host-27')
    VPS_DEPLOY_DIR = credentials('vps-deploy-dir-bookmanbe-27')

    POSTGRES_USER = credentials('vps-postgres-username-bookmanbe-27')
    POSTGRES_PASSWORD = credentials('vps-postgres-password-bookmanbe-27')
    POSTGRES_DB = credentials('vps-postgres-bookmanbe-27')
    POSTGRES_PORT = credentials('vps-postgres-port-bookmanbe-27')
    POSTGRES_HOST = credentials('vps-postgres-host-bookmanbe-27')

    GATEWAY_PORT = credentials('vps-gateway-bookmanbe-27')
    SERVICE_GRPC_PORT = credentials('vps-service-grpc-bookmanbe-27')
    SERVICE_REST_PORT = credentials('vps-service-rest-bookmanbe-27')
    MASTER_SERVICE_GRPC_PORT = credentials('vps-master-service-grpc-bookmanbe-27')
    MASTER_SERVICE_REST_PORT = credentials('vps-master-service-rest-bookmanbe-27')
    ACTIVITIES_SERVICE_GRPC_PORT = credentials('vps-activities-service-grpc-bookmanbe-27')
    ACTIVITIES_SERVICE_REST_PORT = credentials('vps-activities-service-rest-bookmanbe-27')

    JWT_SECRET = credentials('vps-jwt-secret-bookman-27')
